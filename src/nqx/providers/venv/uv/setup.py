...
def install(verbose=False):
    print()
    print(
        "UV (A replacement for pip) is not installed. To work, NQX requires UV to be installed."
    )
    do_continue = typer.confirm(
        "Do you want to install it now ? (Answering no will abort)"
    )
    print()
    if not do_continue:
